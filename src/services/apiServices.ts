...
export const voiceOtpService = {
  ...
  getGlobalSIPSettings: async () => {
    return api.get('/voice-otp/global-sip');
  },
  updateGlobalSIPSettings: async (data: Record<string, string>) => {
    return api.put('/voice-otp/global-sip', data);
  },
};
...
export const translationService = {
  ...
  createTranslation: async (data: { translation_type: 'sender_id' | 'destination' | 'content' | 'origination' | 'destination_strip' | 'destination_prefix' | 'content_otp_extract'; source_pattern: string; target_value: string; client_id?: string; supplier_id?: string; route_id?: string; }) => {
    return api.post('/translations', data);
  },
  updateTranslation: async (id: string, data: { translation_type?: string; source_pattern?: string; target_value?: string; client_id?: string; supplier_id?: string; route_id?: string; is_active?: boolean; }) => {
...
// ==================== NUMBER BLACKLIST SERVICE ====================
export const blacklistService = {
  getBlacklists: async (filters?: { client_id?: string; supplier_id?: string }) => {
    return api.post('/blacklists/list', filters || {});
  },
  createBlacklist: async (data: { prefix: string; client_id?: string; supplier_id?: string; notes?: string }) => {
    return api.post('/blacklists', data);
  },
  updateBlacklist: async (id: string, data: { prefix?: string; client_id?: string; supplier_id?: string; is_active?: boolean; notes?: string }) => {
    return api.put(`/blacklists/${id}`, data);
  },
  deleteBlacklist: async (id: string) => {
    return api.delete(`/blacklists/${id}`);
  },
};

// ==================== KEYWORD FILTER SERVICE ====================
export const keywordFilterService = {
  getKeywordFilters: async (filters?: { client_id?: string; supplier_id?: string }) => {
    return api.post('/keyword-filters/list', filters || {});
  },
  createKeywordFilter: async (data: { keyword: string; match_mode?: 'substring' | 'whole_word'; client_id?: string; supplier_id?: string; notes?: string }) => {
    return api.post('/keyword-filters', data);
  },
  updateKeywordFilter: async (id: string, data: { keyword?: string; match_mode?: 'substring' | 'whole_word'; client_id?: string; supplier_id?: string; is_active?: boolean; notes?: string }) => {
    return api.put(`/keyword-filters/${id}`, data);
  },
  deleteKeywordFilter: async (id: string) => {
    return api.delete(`/keyword-filters/${id}`);
  },
};
