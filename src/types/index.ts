...
export type SMSStatus = 'pending' | 'sent' | 'delivered' | 'failed' | 'expired' | 'rejected' | 'submitted' | 'blocked';

export type BindStatus = 'bound' | 'unbound' | 'binding' | 'error';

export type EntityType = 'client' | 'supplier' | 'rate_plan';
...
export interface SMSLog {
  ...
  // Pre-translation values for before/after comparison
  original_sender_id: string | null;
  original_destination: string | null;
  original_message: string | null;
  applied_translations: string | null;
...
export type TranslationType = 
  | 'sender_id'           // Sender ID transformation (regex)
  | 'destination'         // Destination number transformation (regex)
  | 'content'             // Content transformation (regex)
  | 'origination'         // Origination transformation (regex)
  | 'destination_strip'   // Strip N leading digits from destination
  | 'destination_prefix'  // Add prefix to destination
  | 'content_otp_extract' // Extract OTP from content, replace with template
...
export interface VoiceOTPLog {
  ...
  sip_server_id: number | null;
  status: 'initiated' | 'ringing' | 'answered' | 'completed' | 'failed' | 'busy' | 'no_answer' | 'retrying' | 'timeout';
  dlr_status: string | null;
  error_message: string | null;
...
export interface VoiceOTPGlobalSIP {
  voice_otp_sip_host: string;
  voice_otp_sip_port: string;
  voice_otp_sip_username: string;
  voice_otp_sip_password: string;
  voice_otp_caller_id: string;
  voice_otp_is_e164: string;
  voice_otp_audio_codec: string;
}
...
export interface APIConnector {
  ...
  region: string;
  auth_type: 'API_KEY' | 'BASIC' | 'BEARER' | 'OAUTH2' | 'NONE';
  ...
  is_active: boolean;
  connection_status?: 'untested' | 'connected' | 'failed' | 'testing';
  created_at: string;
  updated_at?: string;
}
...
export interface NumberBlacklist {
  id: string;
  prefix: string;
  client_id: string | null;
  supplier_id: string | null;
...
export type KeywordMatchMode = 'substring' | 'whole_word';
