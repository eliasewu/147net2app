...
function makeLog(overrides: Partial<SMSLog> = {}): SMSLog {
  ...
  return {
    ...
    message: 'Test message', message_parts: 1,
    original_sender_id: null, original_destination: null, original_message: null, applied_translations: null,
    client_rate: 0.01, supplier_rate: 0.005,
    ...
    trunk_id: null, smpp_message_id: null, registered_delivery: null, data_coding: null,
    esm_class: null, channel: null, source: null, dlr_callback_url: null,
...
describe('getRowStyle', () => {
  ...
  it('red for blocked status', () => {
    expect(
      getRowStyle(makeLog({ status: 'blocked', dlr_status: null }))
    ).toBe('bg-red-50 hover:bg-red-100');
  });
