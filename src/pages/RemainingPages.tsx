...
export const RateManagement: React.FC = () => {
  const { rates: allRates, ratePlans, addRate, updateRate, deleteRate, mccmnc, updateRatePlan, deleteRatePlan } = useData();
  const [search, setSearch] = useState('');  const [selectedPlanId, setSelectedPlanId] = useState<string>('all');
  ...
  const [formData, setFormData] = useState({ rate_plan_id: '', mcc: '', mnc: '*', country: '', operator: 'All', rate: 0, effective_from: new Date().toISOString().split('T')[0], effective_to: '', is_active: true });
  const [showAddPlan, setShowAddPlan] = useState(false); const [newPlanName, setNewPlanName] = useState('');
  const [newPlanDesc, setNewPlanDesc] = useState(''); const [newPlanSaving, setNewPlanSaving] = useState(false);
  const [showEditPlan, setShowEditPlan] = useState(false); const [editPlanId, setEditPlanId] = useState('');
  const [editPlanName, setEditPlanName] = useState(''); const [editPlanDesc, setEditPlanDesc] = useState('');
  const [editPlanDefault, setEditPlanDefault] = useState(false); const [editPlanSaving, setEditPlanSaving] = useState(false);
  const [deletePlanId, setDeletePlanId] = useState(''); const [deletePlanName, setDeletePlanName] = useState('');
  const [deletePlanSaving, setDeletePlanSaving] = useState(false);
  const itemsPerPage = 20;
  const planRates = allRates.filter(r => r.entity_type === 'rate_plan');
  const filtered = planRates.filter(r => {
    const m = (r.country || '').toLowerCase().includes(search.toLowerCase())||(r.mcc || '').includes(search);
    const p = selectedPlanId === 'all' || r.entity_id === selectedPlanId;
    return m && p;
  });
  ...
  const getPlanName = (planId: string) => { const p = ratePlans.find(x => x.id === planId); return p ? p.plan_name : 'Unknown Plan'; };
  const openModal = (rate?: Rate) => { if(rate){setEditingRate(rate);setFormData({rate_plan_id:rate.entity_id,mcc:rate.mcc,mnc:rate.mnc,country:rate.country,operator:rate.operator,rate:rate.rate,effective_from:rate.effective_from,effective_to:rate.effective_to||'',is_active:rate.is_active});}else{setEditingRate(null);setFormData({rate_plan_id:selectedPlanId !== 'all' ? selectedPlanId : '',mcc:'',mnc:'*',country:'',operator:'All',rate:0,effective_from:new Date().toISOString().split('T')[0],effective_to:'',is_active:true});}setShowModal(true); };
  const handleSubmit = () => { const {rate_plan_id, ...planFormData} = formData; if(editingRate){if(editingRate.rate!==formData.rate&&editingRate.is_active){updateRate(editingRate.id,{is_active:false,effective_to:new Date().toISOString().split('T')[0]});addRate({...planFormData,currency:'EUR',entity_type:'rate_plan',entity_id:rate_plan_id,effective_to:null});}else{updateRate(editingRate.id,{...planFormData,currency:'EUR',entity_type:'rate_plan',entity_id:rate_plan_id});}}else{const existing=allRates.find(r=>r.entity_type==='rate_plan'&&r.entity_id===rate_plan_id&&r.mcc===formData.mcc&&r.mnc===formData.mnc&&r.is_active);if(existing){updateRate(existing.id,{is_active:false,effective_to:new Date().toISOString().split('T')[0]});}addRate({...planFormData,currency:'EUR',entity_type:'rate_plan',entity_id:rate_plan_id,effective_to:formData.effective_to||null});}setShowModal(false); };
  const handleBulkAdd = () => { bulkText.trim().split('\n').forEach(line=>{const p=line.split(',').map(x=>x.trim());if(p.length>=6){const planId=p[0],mcc=p[1],mnc=p[2]||'*',country=p[3],operator=p[4]||'All',newRate=parseFloat(p[5])||0;const existing=allRates.find(r=>r.entity_type==='rate_plan'&&r.entity_id===planId&&r.mcc===mcc&&r.mnc===mnc&&r.is_active);if(existing){updateRate(existing.id,{is_active:false,effective_to:new Date().toISOString().split('T')[0]});}addRate({entity_type:'rate_plan',entity_id:planId,mcc,mnc,country,operator,rate:newRate,currency:'EUR',effective_from:new Date().toISOString().split('T')[0],effective_to:null,is_active:true});}});setShowBulkModal(false);setBulkText(''); };
  const handleBulkDelete = () => { selectedRates.forEach(id=>deleteRate(id));setSelectedRates([]); };
  const handleAddPlan = async () => { if (!newPlanName.trim()) return; setNewPlanSaving(true); try { const res = await api.post('/rate-plans', { plan_name: newPlanName.trim(), description: newPlanDesc.trim(), is_default: false }); setShowAddPlan(false); setNewPlanName(''); setNewPlanDesc(''); if (res?.data?.id) setSelectedPlanId(String(res.data.id)); } finally { setNewPlanSaving(false); } };
  const toggleSelect = (id:string) => setSelectedRates(p=>p.includes(id)?p.filter(i=>i!==id):[...p,id]);
  const handleEditPlan = async () => { if (!editPlanName.trim()) return; setEditPlanSaving(true); try { await updateRatePlan(editPlanId, { plan_name: editPlanName.trim(), description: editPlanDesc.trim(), is_default: editPlanDefault }); setShowEditPlan(false); } finally { setEditPlanSaving(false); } };
  const handleDeletePlan = async () => { setDeletePlanSaving(true); try { await deleteRatePlan(deletePlanId); setDeletePlanId(''); setDeletePlanName(''); if (selectedPlanId === deletePlanId) setSelectedPlanId('all'); } finally { setDeletePlanSaving(false); } };
  const openEditPlanModal = () => { const p = ratePlans.find(x => x.id === selectedPlanId); if (!p) return; setEditPlanId(p.id); setEditPlanName(p.plan_name); setEditPlanDesc(p.description || ''); setEditPlanDefault(p.is_default); setShowEditPlan(true); };
  const openDeletePlanModal = () => { const p = ratePlans.find(x => x.id === selectedPlanId); if (!p) return; setDeletePlanId(p.id); setDeletePlanName(p.plan_name); };
  ...
  const cols = [
    ...
    {key:'plan',header:'Rate Plan',render:(r:Rate)=><span className="text-sm font-medium">{getPlanName(r.entity_id)}</span>},
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-gray-800">Rate Management</h1><p className="text-gray-500 mt-1">Manage rates by rate plan — select a plan to view and edit its rates</p></div><div className="flex gap-2"><Button variant="secondary" icon={<Upload size={16}/>} onClick={()=>setShowBulkModal(true)}>Bulk Update</Button><Button variant="secondary" icon={<Download size={16}/>} onClick={()=>exportCSV('all_rates_export.csv',['rate_plan','mcc','mnc','country','operator','rate','currency','effective_from','is_active'],displayed.map(r=>[getPlanName(r.entity_id),r.mcc,r.mnc,r.country,r.operator,r.rate.toFixed(6),r.currency,r.effective_from,String(r.is_active)]))}>Export CSV</Button><Button variant="secondary" icon={<Download size={16}/>} onClick={()=>exportExcel('all_rates_export.xlsx','Rate Management',['Rate Plan','MCC','MNC','Country','Operator','Rate','Currency','Effective From','Is Active'],displayed.map(r=>[getPlanName(r.entity_id),r.mcc,r.mnc,r.country,r.operator,r.rate.toFixed(6),r.currency,r.effective_from,String(r.is_active)]))}>Export Excel</Button><Button icon={<Plus size={18}/>} onClick={()=>openModal()}>Add Rate</Button></div></div>
    ...
    <Card><div className="flex flex-wrap items-center gap-3"><div className="min-w-[280px]"><Select label="Rate Plan" value={selectedPlanId} onChange={e => { setSelectedPlanId(e.target.value); setCurrentPage(1); }} options={[{value:'all',label:`All Plans (${planRates.length} rates)`},...ratePlans.map(p=>({value:p.id,label:`${p.plan_name} (${planRates.filter(r=>r.entity_id===p.id).length} rates)`}))]} /></div>{!showAddPlan ? <button onClick={() => setShowAddPlan(true)} className="p-1.5 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 transition-colors flex-shrink-0" title="Quick-add rate plan"><Plus size={16} /></button> : <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2"><input type="text" placeholder="Plan name..." value={newPlanName} onChange={e => setNewPlanName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleAddPlan(); if (e.key === 'Escape') { setShowAddPlan(false); setNewPlanName(''); setNewPlanDesc(''); } }} className="px-2 py-1 border border-gray-300 rounded text-sm w-36 focus:outline-none focus:ring-1 focus:ring-blue-400" autoFocus /><input type="text" placeholder="Desc (optional)" value={newPlanDesc} onChange={e => setNewPlanDesc(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleAddPlan(); if (e.key === 'Escape') { setShowAddPlan(false); setNewPlanName(''); setNewPlanDesc(''); } }} className="px-2 py-1 border border-gray-300 rounded text-sm w-28 focus:outline-none focus:ring-1 focus:ring-blue-400" /><button onClick={handleAddPlan} disabled={!newPlanName.trim() || newPlanSaving} className={`px-3 py-1 rounded text-xs font-medium transition-colors ${!newPlanName.trim() ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>{newPlanSaving ? '...' : 'Save'}</button><button onClick={() => { setShowAddPlan(false); setNewPlanName(''); setNewPlanDesc(''); }} className="p-1 rounded text-gray-400 hover:text-gray-600"><X size={14} /></button></div>}{selectedPlanId !== 'all' && !showAddPlan && <><button onClick={openEditPlanModal} className="p-1.5 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors flex-shrink-0" title="Edit rate plan"><Edit size={14} /></button><button onClick={openDeletePlanModal} className="p-1.5 rounded-full bg-red-50 text-red-500 hover:bg-red-100 transition-colors flex-shrink-0" title="Delete rate plan"><Trash2 size={14} /></button></>}</div></Card>
    ...
    <Modal isOpen={showModal} onClose={()=>setShowModal(false)} title={editingRate?'Edit Rate':'Add Rate'} footer={<div className="flex justify-end gap-3"><Button variant="secondary" onClick={()=>setShowModal(false)}>Cancel</Button><Button onClick={handleSubmit}>{editingRate?'Update':'Add'}</Button></div>}><div className="space-y-4"><Select label="Rate Plan" value={formData.rate_plan_id} onChange={e=>setFormData(p=>({...p,rate_plan_id:e.target.value}))} options={[{value:'',label:'Select...'},...ratePlans.map(p=>({value:p.id,label:p.plan_name}))]} required/><div className="grid grid-cols-2 gap-4"><Select label="MCC" value={formData.mcc} onChange={e=>{const entry=mccmnc.find(m=>m.mcc===e.target.value);setFormData(p=>({...p,mcc:e.target.value,country:entry?.country||''}));}} options={[{value:'',label:'Select'},...Array.from(new Set(mccmnc.map(m=>m.mcc))).map(mcc=>({value:mcc,label:`${mcc} - ${mccmnc.find(m=>m.mcc===mcc)?.country||''}`}))]} required/><Input label="MNC" value={formData.mnc} onChange={e=>setFormData(p=>({...p,mnc:e.target.value}))} placeholder="*"/></div><Input label="Rate (EUR)" type="number" step="0.0001" value={formData.rate} onChange={e=>setFormData(p=>({...p,rate:parseFloat(e.target.value)}))} required/><div className="grid grid-cols-2 gap-4"><Input label="Effective From" type="date" value={formData.effective_from} onChange={e=>setFormData(p=>({...p,effective_from:e.target.value}))}/><Input label="Effective To" type="date" value={formData.effective_to} onChange={e=>setFormData(p=>({...p,effective_to:e.target.value}))}/></div></div></Modal>
    <Modal isOpen={showBulkModal} onClose={()=>setShowBulkModal(false)} title="Bulk Update Rates" size="lg" footer={<div className="flex justify-end gap-3"><Button variant="secondary" onClick={()=>setShowBulkModal(false)}>Cancel</Button><Button onClick={handleBulkAdd}>Import</Button></div>}><div className="space-y-4"><div className="bg-yellow-50 p-4 rounded-lg"><p className="text-sm text-yellow-700 font-medium mb-2">CSV: rate_plan_id, mcc, mnc, country, operator, rate</p><code className="text-xs text-yellow-600">PLAN_UUID, 310, *, United States, All, 0.0100</code></div><textarea value={bulkText} onChange={e=>setBulkText(e.target.value)} className="w-full h-64 px-4 py-3 border border-gray-300 rounded-lg font-mono text-sm" placeholder="Paste CSV..."/></div></Modal>
    <Modal isOpen={showEditPlan} onClose={()=>setShowEditPlan(false)} title="Edit Rate Plan" footer={<div className="flex justify-end gap-3"><Button variant="secondary" onClick={()=>setShowEditPlan(false)}>Cancel</Button><Button onClick={handleEditPlan} loading={editPlanSaving} disabled={!editPlanName.trim()}>Update</Button></div>}><div className="space-y-4"><Input label="Plan Name" value={editPlanName} onChange={e=>setEditPlanName(e.target.value)} required/><Input label="Description" value={editPlanDesc} onChange={e=>setEditPlanDesc(e.target.value)}/><label className="flex items-center gap-2"><input type="checkbox" checked={editPlanDefault} onChange={e=>setEditPlanDefault(e.target.checked)} className="w-4 h-4 rounded"/><span className="text-sm">Set as default plan</span></label></div></Modal>
    <Modal isOpen={!!deletePlanId} onClose={()=>{setDeletePlanId('');setDeletePlanName('');}} title="Delete Rate Plan" footer={<div className="flex justify-end gap-3"><Button variant="secondary" onClick={()=>{setDeletePlanId('');setDeletePlanName('');}}>Cancel</Button><Button variant="danger" onClick={handleDeletePlan} loading={deletePlanSaving}>Delete</Button></div>}><p className="text-gray-600">Are you sure you want to delete <strong>{deletePlanName}</strong>? This will also delete all its rates and cannot be undone.</p></Modal></div>);
};
...
export const PlatformSettings: React.FC = () => {
  ...
  const handleSave = async () => { setSaving(true); await new Promise(r=>setTimeout(r,500)); updateSMTPConfig(smtp); setSaving(false); alert('Settings saved to database!'); };
...
export const BackupPage: React.FC = () => {
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-gray-800">Backup & Restore</h1><p className="text-gray-500 mt-1">{backups.length} backups from database</p></div><div className="flex gap-2"><Button variant="secondary" onClick={reload}>Refresh</Button><Button icon={<Plus size={18}/>} onClick={()=>alert('Schedule via cron / pg_dump on the host.')} disabled>Create Backup</Button></div></div><Card title="Backup Files" noPadding>{loading?<p className="text-gray-500 text-center py-8">Loading…</p>:backups.length===0?<p className="text-gray-500 text-center py-8">No backups registered yet.</p>:<Table columns={cols} data={backups} keyExtractor={(b:any)=>b.id||b.name}/>}</Card></div>);
};
