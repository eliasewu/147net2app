...
import { formatDuration } from '../../utils/smsHelpers';
...
export const TestSMS: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const client = clients.find(c => String(c.id) === formData.client_id);
    ...
    await new Promise(r => setTimeout(r, 500));
    const rp = routePlans.find(p => String(p.id) === formData.route_plan_id);
    if (!rp) {
    ...
    const tick = async (): Promise<void> => {
      ...
      try {
        ...
        if (status === 'delivered' || dlr_status === 'DELIVERED') {
          setResults(prev => prev.map(r => r.id === newResult.id ? { ...r, status: 'delivered' as const, dlr_status: dlr_status || 'DELIVRD', supplier: supplierName, latency: Date.now() - startedAt } : r));
          setValidationLog(prev => [...prev, '[DLR] ✅ Delivered']);
          setLoading(false);
          return;
        }
        if (status === 'failed' || dlr_status === 'FAILED' || dlr_status === 'REJECTED') {
          setResults(prev => prev.map(r => r.id === newResult.id ? { ...r, status: 'failed' as const, dlr_status: dlr_status, error: dlr?.data?.error_message || dlr?.error_message || 'Delivery failed', supplier: supplierName } : r));
