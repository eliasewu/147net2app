...
// ============================================================
// SERVER IDENTITY — hardcoded so license is always bound to
// this physical host. Changing the IP or MAC invalidates the
// license activation.
// ============================================================
const SERVER_IP = '147.135.128.43';
const SERVER_MAC = '0c:c4:7a:07:6a:24';

async function detectServerInfo(): Promise<{ ip: string; mac: string }> {
  return { ip: SERVER_IP, mac: SERVER_MAC };
}
...
const defaultTenants: Tenant[] = [
  { id:'1', name:'TechCorp Global', code:'TCG', status:'active', features:{ smpp:true, http:true, whatsapp:false, telegram:false, rcs:false, voice_otp:false }, limits:{ max_sms_monthly:500, max_tps:20 }, usage:{ sms_this_month:45, current_tps:2, sms_today:8 },  ip:SERVER_IP, mac:SERVER_MAC, license_expiry: new Date(Date.now()+30*86400000).toISOString().split('T')[0], created_at:'2024-06-01' },
];
...
export const License: React.FC = () => {
  ...
  // Seed server identity on mount — always force the real IP/MAC
  useEffect(() => {
    setServerIP(SERVER_IP);
    setServerMAC(SERVER_MAC);
    setLicense(prev => {
      if (!prev.system_ip || prev.system_ip === '192.168.1.100' || prev.system_ip === '127.0.0.1') {
        return { ...prev, system_ip: SERVER_IP, system_mac: SERVER_MAC };
      }
      return prev;
    });
  }, []);
