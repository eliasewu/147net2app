...
export const Dashboard: React.FC = () => {
  ...
  const blockedSMS = smsLogs.filter(l => l.status === 'blocked').length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" size="sm" icon={<Download size={14}/>} onClick={()=>exportCSV('dashboard_stats.csv',['Metric','Value'],[['Total SMS',String(smsLogs.length)],['Delivered SMS',String(smsLogs.filter(l=>l.status==='delivered').length)],['Failed SMS',String(smsLogs.filter(l=>l.status==='failed').length)],
['Blocked SMS',String(blockedSMS)],
['Active Clients',clients.filter(c=>c.status==='active').length+'/'+clients.length],
['Active Binds',boundCount+'/'+(boundCount+unboundCount)],
['Consecutive Failures',String(consecutiveFails)],['Low Balance Clients',String(lowBalanceClients.length)],['Channel Disconnects',String(blockedSuppliers.length)],['Pending Invoices',String(recentInvoices.length)],['Total Clients',String(clients.length)],['Total Suppliers',String(suppliers.length)],['Total Invoices',String(invoices.length)],['Total Payments',String(payments.length)]])}>Stats CSV</Button>
          <Button variant="secondary" size="sm" icon={<Download size={14}/>} onClick={()=>exportExcel('dashboard_stats.xlsx','Dashboard Stats',['Metric','Value'],[['Total SMS',String(smsLogs.length)],['Delivered SMS',String(smsLogs.filter(l=>l.status==='delivered').length)],['Failed SMS',String(smsLogs.filter(l=>l.status==='failed').length)],
['Blocked SMS',String(blockedSMS)],
['Active Clients',clients.filter(c=>c.status==='active').length+'/'+clients.length],
['Active Binds',boundCount+'/'+(boundCount+unboundCount)],
['Consecutive Failures',String(consecutiveFails)],['Low Balance Clients',String(lowBalanceClients.length)],['Channel Disconnects',String(blockedSuppliers.length)],['Pending Invoices',String(recentInvoices.length)],['Total Clients',String(clients.length)],['Total Suppliers',String(suppliers.length)],['Total Invoices',String(invoices.length)],['Total Payments',String(payments.length)]])}>Stats Excel</Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard title="Blocked" value={formatNumber(blockedSMS)} icon={<AlertTriangle size={24}/>} color="red"/>
            ...
            {recentSMS.map(sms => (
              <div key={sms.id} className="px-4 py-2.5 flex items-center justify-between hover:bg-gray-50">
                <div className="flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${sms.status==='delivered'?'bg-green-500':sms.status==='failed'||sms.status==='blocked'?'bg-red-500':sms.status==='rejected'?'bg-orange-500':'bg-yellow-500'}`}/>
                  <div>
