...
export const AsteriskConfig: React.FC = () => {
  ...
  const tryInstall = async () => {
    ...
    } catch (e: any) { alert('Install error: ' + e.message); }
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      setGen(r?.data || null);
      await loadChecklist();
    } catch (e: any) { alert('Regenerate error: ' + e.message); }
    finally { setBusy(null); }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      alert('Saved. Run on the Asterisk host: asterisk -rx "reload"');
      await loadHealth();
  ...
  const saveServer = async () => {
    ...
    try {
      ...
      closeModal();
    } catch (e: any) { alert('Save server failed: ' + (e?.message || 'unknown')); }
    finally { setBusy(null); }
  };

  const archiveServer = async (s: SipServer) => {
    if (!confirm(`Archive server "${s.name}"? Existing call history is preserved.`)) return;
    try { await api.delete(`/asterisk/servers/${s.id}`); await loadServers(); await loadHealth(); }
