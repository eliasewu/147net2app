...
function originate(serverIdOrOpts, paramsMaybe) {
  ...
  // Dial the actual destination via the PJSIP trunk, and when the call is
  // answered, bridge to extension 's' in the dialplan context which plays
  // SayDigits(${OTP_CODE}) in the caller's language.
  const dest = String(params.destination || '').replace(/[^0-9+]/g, '');
  const headers = {
    Action: 'Originate',
    Channel: dest ? `PJSIP/${dest}@net2app-platform` : `Local/s@${server.dialplan_context || 'net2app-otp'}/n`,
