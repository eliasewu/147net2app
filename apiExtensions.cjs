...
app.post('/api/sms/validate', auth, async (req, res) => {
  try {
    ...
    if (mcc) {
      // Route maps removed — use resolveRouteForClient instead
      const resolved = await pool.resolveRouteForClient(client_id, destination);
      if (resolved) {
        supplier = { id: resolved.supplier_id, code: resolved.supplier_code };
        routeId = resolved.route_id;
        routeName = resolved.route_name;
      }
    }
...
app.post('/api/rates/notify', auth, roles('super_admin','admin','billing'), async (req, res) => {
  try {
    const { entity_type, entity_id, rate_ids = [], destination, old_rate, new_rate, effective_date, operator, change_pct } = req.body || {};
    if (!entity_type || !entity_id) return res.status(400).json({ error: 'entity_type and entity_id required' });
    const entityIdNum = parseInt(entity_id, 10);

    // Resolve entities to notify based on entity_type
    let entities = [];

    if (entity_type === 'rate_plan') {
      const clients = await pool.query(
        'SELECT id, company_name, email FROM clients WHERE rate_plan_id = $1 AND status = $2',
        [entityIdNum, 'active']
      );
      const suppliers = await pool.query(
        'SELECT id, company_name, email FROM suppliers WHERE rate_plan_id = $1 AND status = $2',
        [entityIdNum, 'active']
      );
      entities = [
      ...
      const r = await pool.query('SELECT id, company_name, email FROM clients WHERE id = $1', [entityIdNum]);
      entities = r.rows.map((row) => ({ id: row.id, email: row.email, company_name: row.company_name, type: 'client' }));
    } else if (entity_type === 'supplier') {
      const r = await pool.query('SELECT id, company_name, email FROM suppliers WHERE id = $1', [entityIdNum]);
      entities = r.rows.map((row) => ({ id: row.id, email: row.email, company_name: row.company_name, type: 'supplier' }));
    } else {
      return res.status(400).json({ error: 'Invalid entity_type. Use client, supplier, or rate_plan.' });
    }

    if (!entities.length) return res.json({ success: true, message: 'No entities to notify', sent: 0 });
    ...
    const results = [];
    const destLabel = destination || rates[0]?.country || 'Rate Plan';
    ...
    for (const ent of entities) {
      ...
      // Create notification in DB with correct entity_type
      const pctLabel = change_pct === 'New' ? 'New destination' : (change_pct || (old_rate > 0 ? `${((new_rate - old_rate) / old_rate * 100).toFixed(1)}%` : 'New'));
      const message = `Rate update for ${ent.company_name}: ${destLabel} — ${pctLabel} (${old_rate > 0 ? '€' + Number(old_rate).toFixed(4) + ' → ' : ''}€${Number(new_rate).toFixed(4)})`;
      await pool.query(
        ...
        ['Rate update', message, ent.type, ent.company_name, entity_id, ent.email]
      ).catch(e => console.warn('[notify] notification insert failed:', e.message));

      // Send email
      if (_emailService && ent.email && destination && new_rate != null) {
        try {
          const r = await _emailService.sendRateChangeEmail({
        ...
        results.push({ email: ent.email, sent: true, notified: true });
      }
    }

    const sentCount = results.filter(r => r.sent).length;
    const notified = entities[0]?.email || null;
    const emailSent = sentCount > 0;
    res.json({ success: true, notified, rate_count: rates.length, email_sent: emailSent, sent: sentCount, total: results.length, results });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/invoices/:id/send', auth, roles('super_admin','admin','billing'), async (req, res) => {
  try {
    ...
    const invoiceEmails = emails.length ? emails : (inv.entity_email ? [inv.entity_email] : []);
    if (_emailService && invoiceEmails.length) {
      _emailService.sendNotificationEmail({
        template_name: 'Invoice Generated',
        variables: {
          client_name: inv.entity_name || 'Client',
      ...
      }).catch(e => console.warn('[email] Invoice email failed:', e.message));
    }
...
// -----------------------------------------------------------------
// VOICE OTP GLOBAL SIP SETTINGS (stored in platform_settings)
// -----------------------------------------------------------------
app.get('/api/voice-otp/global-sip', auth, async (req, res) => {
  try {
    const r = await pool.query("SELECT key, value FROM platform_settings WHERE key LIKE 'voice_otp_%'");
    const settings = {};
...
app.put('/api/voice-otp/global-sip', auth, roles('super_admin','admin'), async (req, res) => {
  try {
    const keys = ['voice_otp_sip_host','voice_otp_sip_port','voice_otp_sip_username','voice_otp_sip_password','voice_otp_caller_id','voice_otp_is_e164','voice_otp_audio_codec'];
    for (const k of keys) {
...
app.post('/api/voice-otp/configs', auth, roles('super_admin','admin'), async (req, res) => {
  try {
    const b = req.body || {};
    if (!b.language || !b.country_prefix) return res.status(400).json({ error: 'language and country_prefix required' });
    if (b.retry_count != null && (b.retry_count < 1 || b.retry_count > 4)) return res.status(400).json({ error: 'retry_count must be 1-4' });
    if (b.play_count != null && (b.play_count < 1 || b.play_count > 3)) return res.status(400).json({ error: 'play_count must be 1-3' });
    const allowed = ['language','language_code','country_prefix','primary_language_code','secondary_language_code','primary_greeting_text','primary_retry_text','greeting_text','retry_text','retry_count','play_count','sip_host','sip_port','caller_id','is_active'];
    const keys = allowed.filter(k => b[k] !== undefined);
    if (!keys.includes('language_code')) { keys.push('language_code'); b.language_code = b.primary_language_code || 'en'; }
    if (!keys.includes('greeting_text')) { keys.push('greeting_text'); b.greeting_text = b.primary_greeting_text || ''; }
    if (!keys.includes('retry_text')) { keys.push('retry_text'); b.retry_text = b.primary_retry_text || ''; }
    const vals = keys.map(k => b[k]);
    const ph = keys.map((_, i) => '$' + (i + 1)).join(',');
    const r = await pool.query(`INSERT INTO voice_otp_configs (${keys.join(',')}) VALUES (${ph}) RETURNING *`, vals);
    res.json({ success: true, data: r.rows[0] });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.delete('/api/voice-otp/configs/:id', auth, roles('super_admin','admin'), async (req, res) => {
  try {
    const r = await pool.query('DELETE FROM voice_otp_configs WHERE id=$1 RETURNING *', [req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/voice-otp/configs/:id', auth, roles('super_admin','admin','support'), async (req, res) => {
  try {
    const allowed = ['language','language_code','country_prefix','primary_language_code','secondary_language_code','primary_greeting_text','primary_retry_text','greeting_text','retry_text','greeting_audio_url','audio_files','retry_count','play_count','sip_host','sip_port','sip_username','sip_password','caller_id','is_active'];
...
// ----- VOICE OTP: seed defaults — idempotent; creates country groups
    ...
    for (const g of groups) {
      ...
      await pool.query(
        ...
        [g.name, g.primary_language_code || 'en', g.country_prefix || '',
         g.primary_language_code || 'en', g.secondary_language_code || '',
         '', '',
         g.retry_count ?? 3, g.play_count ?? 2,
         g.is_active !== false]
      );
      inserted++;
    }
...
// ----- VOICE OTP: audio upload (per digit, per language group).
// Accepts mp3 OR wav — converts to wav via ffmpeg.
app.post('/api/voice-otp/audio-upload', auth, roles('super_admin','admin','support'), audioUpload.single('audio'), async (req, res) => {
  try {
    ...
    if (group_id) {
      ...
      return res.json({ success: true, data: { url: relUrl, size_bytes: wav.length, language_code, digit, group_id } });
    }
...
//   sender_id, destination, content, origination — regex-based
//   destination_strip  — strip N leading digits (source_pattern = digit count)
//   destination_prefix — add prefix (source_pattern = prefix string)
//   content_otp_extract — extract OTP via regex, replace with template
// -----------------------------------------------------------------
...
app.post('/api/translations/apply', auth, async (req, res) => {
  try {
    ...
    for (const r of rulesR.rows) {
      try {
        // --- DESTINATION STRIP (non-regex): strip N leading digits ---
        if (r.translation_type === 'destination_strip') {
          ...
          if (stripCount > 0 && stripCount < destination.length) {
            const oldDest = destination;
            destination = destination.substring(stripCount);
            applied.push({ id: r.id, type: r.translation_type, rule: `strip ${stripCount} digits`, before: oldDest, after: destination });
          }
          continue;
        }
        // --- DESTINATION PREFIX (non-regex): add prefix ---
        if (r.translation_type === 'destination_prefix') {
          const prefix = r.source_pattern || '';
          if (prefix && !destination.startsWith(prefix)) {
            const oldDest = destination;
            destination = prefix + destination;
            applied.push({ id: r.id, type: r.translation_type, rule: `add prefix "${prefix}"`, before: oldDest, after: destination });
          }
          continue;
        }
        // --- CONTENT OTP EXTRACT: extract OTP via regex, replace with template ---
        if (r.translation_type === 'content_otp_extract') {
          ...
          if (match) {
            ...
            const newMsg = template.replace(/\{otp\}/gi, otpValue);
            // If template is just "{otp}", forward ONLY the OTP
            const oldMsg = message;
            message = newMsg;
            applied.push({ id: r.id, type: r.translation_type, rule: r.source_pattern, otp: otpValue, before: oldMsg, after: message });
          }
          continue;
        }
        // --- REGEX-BASED TYPES ---
...
app.post('/api/translations/test', auth, async (req, res) => {
  try {
    const b = req.body || {};
    const t = b.translation_type || 'destination';
    const input = b.test_input || '';
    // Non-regex types
    if (t === 'destination_strip') {
      const n = parseInt(b.source_pattern, 10) || 0;
      const result = n > 0 ? input.substring(n) : input;
      return res.json({ success: true, data: { input, output: result, matches: n > 0 && input.length >= n, stripped: n } });
    }
    if (t === 'destination_prefix') {
      const prefix = b.source_pattern || '';
      const result = prefix ? prefix + input : input;
      return res.json({ success: true, data: { input, output: result, matches: !!prefix, prefix } });
    }
    if (t === 'content_otp_extract') {
      try {
        const re = new RegExp(b.source_pattern, 'gi');
        const match = input.match(re);
        if (match) {
          const otp = match[0];
          const template = b.target_value || '{otp}';
          const result = template.replace(/\{otp\}/gi, otp);
          return res.json({ success: true, data: { input, output: result, matches: true, otp_extracted: otp } });
        }
        return res.json({ success: true, data: { input, output: input, matches: false, otp_extracted: null } });
      } catch (re) {
        return res.status(400).json({ error: 'invalid regex: ' + re.message });
      }
    }
    // Regex-based types
    try {
      const re = new RegExp(b.source_pattern);
      const result = input.replace(re, b.target_value || '');
      res.json({ success: true, data: { input, output: result, matches: !!input.match(re) } });
    } catch (re) {
...
app.post('/api/api-connectors', auth, roles('super_admin','admin'), async (req, res) => {
  try {
    ...
    const allowed = ['name','provider','connector_type','region','auth_type','http_method','api_key','api_secret','send_url','dlr_url','dlr_webhook_secret','dlr_status_mapping','submit_pattern','dlr_pattern','dlr_value','test_payload','params','send_body_template','send_response_pattern','dlr_query_url','dlr_query_params','dlr_response_pattern','is_active'];
...
app.put('/api/api-connectors/:id', auth, roles('super_admin','admin'), async (req, res) => {
  try {
    const b = req.body || {};
    const allowed = ['name','provider','connector_type','region','auth_type','http_method','api_key','api_secret','send_url','dlr_url','dlr_webhook_secret','dlr_status_mapping','submit_pattern','dlr_pattern','dlr_value','test_payload','params','send_body_template','send_response_pattern','dlr_query_url','dlr_query_params','dlr_response_pattern','is_active','connection_status'];
    const keys = allowed.filter(k => b[k] !== undefined);
    if (!keys.length) return res.json({ success: true });
    const sets = keys.map((k, i) => `${k}=$${i+1}`).join(',');
    const vals = keys.map(k => b[k]);
...
function isUrlSafeForOutbound(urlStr) {
  try {
    ...
    if (/^(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.|169\.254\.|fc00:|fd00:|fe80:)/.test(ip)) return false;
...
// ----- API CONNECTOR: test-send (fire a real HTTP request with dummy data) -----
app.post('/api/api-connectors/test-send', auth, async (req, res) => {
  try {
    const { send_url, http_method, send_body_template, submit_pattern, api_key, auth_type, params } = req.body || {};
    if (!send_url) return res.status(400).json({ error: 'send_url required' });
    if (!isUrlSafeForOutbound(send_url)) return res.status(400).json({ error: 'URL not allowed (private/internal IPs blocked)' });

    // Build test payload with dummy data
    const testDest = '+1234567890';
    const testSender = 'TEST';
    const testText = 'Hello from NET2APP Hub test';

    let url = send_url;
    const headers = { 'Content-Type': 'application/json', 'User-Agent': 'NET2APP-Hub/1.0' };
    if (auth_type === 'API_KEY' && api_key) headers['X-API-Key'] = api_key;
    if (auth_type === 'BEARER' && api_key) headers['Authorization'] = `Bearer ${api_key}`;
    if (auth_type === 'BASIC' && api_key) {
      const parts = api_key.includes(':') ? api_key.split(':') : [api_key, ''];
      headers['Authorization'] = `Basic ${Buffer.from(parts[0] + ':' + parts[1]).toString('base64')}`;
    }
    ...
    if (method === 'GET') {
      // Append query params to URL, replacing template vars
      const paramList = (params || '').split(',').map(p => p.trim()).filter(Boolean);
      const qp = new URLSearchParams();
      paramList.forEach(p => {
        const val = p.toLowerCase().includes('api') || p.toLowerCase().includes('key') ? (api_key || 'test')
          : p.toLowerCase().includes('msisdn') || p.toLowerCase().includes('to') || p.toLowerCase().includes('phone') || p.toLowerCase().includes('number') ? testDest
          : p.toLowerCase().includes('text') || p.toLowerCase().includes('msg') || p.toLowerCase().includes('message') || p.toLowerCase().includes('code') ? testText
          : p.toLowerCase().includes('from') || p.toLowerCase().includes('sender') ? testSender
          : 'test';
        qp.append(p, val);
      });
      url += (url.includes('?') ? '&' : '?') + qp.toString();
    } else {
      // POST/PUT — use body template or build from params
      if (send_body_template) {
        body = send_body_template
          .replace(/\{\{to\}\}/gi, testDest).replace(/\{\{destination\}\}/gi, testDest).replace(/\{\{msisdn\}\}/gi, testDest)
          .replace(/\{\{text\}\}/gi, testText).replace(/\{\{message\}\}/gi, testText).replace(/\{\{code\}\}/gi, testText)
          .replace(/\{\{from\}\}/gi, testSender).replace(/\{\{sender\}\}/gi, testSender);
      } else {
        const payload = { to: testDest, from: testSender, text: testText, message: testText };
    ...
    try {
      if (submit_pattern) {
        parsedSuccess = new RegExp(submit_pattern, 'i').test(respText);
      } else {
        parsedSuccess = fetchResp.ok;
      }
    } catch { parsedSuccess = fetchResp.ok; }

    // Filter safe response headers only
    const safeHeaderKeys = ['content-type','content-length','date','server','x-request-id','x-message-id','x-rate-limit-remaining'];
    const respHeaders = {};
    safeHeaderKeys.forEach(h => { const v = fetchResp.headers.get(h); if (v) respHeaders[h] = v; });

    res.json({
      success: true,
      request: { url, method, headers: Object.keys(headers), body: body ? body.substring(0, 500) : null },
      response: { status: fetchResp.status, statusText: fetchResp.statusText, headers: respHeaders, body: respText.substring(0, 2000) },
      latency_ms: latencyMs,
      parsed_success: parsedSuccess,
      extracted_message_id: extractedMsgId,
    });
  } catch (e) {
    res.json({ success: false, error: e.message, phase: e.cause ? 'connect' : 'request' });
  }
});

// ----- API CONNECTOR: test-dlr (fire a DLR query with a fake message_id) -----
app.post('/api/api-connectors/test-dlr', auth, async (req, res) => {
  try {
    const { dlr_query_url, dlr_query_params, api_key, auth_type, dlr_response_pattern } = req.body || {};
    if (!dlr_query_url) return res.status(400).json({ error: 'dlr_query_url required' });
    if (!isUrlSafeForOutbound(dlr_query_url)) return res.status(400).json({ error: 'URL not allowed (private/internal IPs blocked)' });

    const testMsgId = 'TEST_DLR_' + Date.now();
    let url = dlr_query_url.replace(/\{\{message_id\}\}/gi, testMsgId).replace(/\{\{msg_id\}\}/gi, testMsgId);

    const headers = { 'Content-Type': 'application/json', 'User-Agent': 'NET2APP-Hub/1.0' };
    if (auth_type === 'API_KEY' && api_key) headers['X-API-Key'] = api_key;
    if (auth_type === 'BEARER' && api_key) headers['Authorization'] = `Bearer ${api_key}`;

    const paramList = (dlr_query_params || '').split(',').map(p => p.trim()).filter(Boolean);
    if (paramList.length > 0 && !url.includes('?')) {
      const qp = new URLSearchParams();
      paramList.forEach(p => { qp.append(p, testMsgId); });
      url += '?' + qp.toString();
    }

    const startMs = Date.now();
    const fetchResp = await fetch(url, { headers, signal: AbortSignal.timeout(15000) });
    const respText = await fetchResp.text();
    const latencyMs = Date.now() - startMs;

    let parsedOk = false;
    try {
      if (dlr_response_pattern) {
        parsedOk = new RegExp(dlr_response_pattern, 'i').test(respText);
      } else {
        parsedOk = fetchResp.ok;
      }
    } catch { parsedOk = fetchResp.ok; }

    res.json({
      success: true,
      request: { url, method: 'GET', headers: Object.keys(headers) },
      response: { status: fetchResp.status, statusText: fetchResp.statusText, body: respText.substring(0, 2000) },
      latency_ms: latencyMs,
      parsed_dlr_ok: parsedOk,
    });
  } catch (e) {
    res.json({ success: false, error: e.message, phase: 'dlr_query' });
  }
});
...
console.log('[apiExtensions] loaded:', [
  ...
  // ============================================================
  // NUMBER BLACKLISTS — CRUD + list
  // ============================================================
  app.post('/api/blacklists/list', auth, async (req, res) => {
    try {
      let q = 'SELECT * FROM number_blacklists WHERE 1=1';
      const p = []; let i = 1;
      if (req.body?.client_id) { q += ` AND client_id=$${i++}`; p.push(req.body.client_id); }
      if (req.body?.supplier_id) { q += ` AND supplier_id=$${i++}`; p.push(req.body.supplier_id); }
  ...
  app.post('/api/blacklists', auth, roles('super_admin','admin','support'), async (req, res) => {
    try {
      const { prefix, client_id, supplier_id, notes } = req.body || {};
      if (!prefix) return res.status(400).json({ error: 'prefix required' });
      const r = await pool.query(
        `INSERT INTO number_blacklists (prefix, client_id, supplier_id, is_active, notes)
         VALUES ($1, $2, $3, true, $4) RETURNING *`,
        [prefix, client_id || null, supplier_id || null, notes || null]
      );
      res.json({ success: true, data: r.rows[0] });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.put('/api/blacklists/:id', auth, roles('super_admin','admin','support'), async (req, res) => {
    try {
      const sets = []; const vals = []; let i = 1;
      const f = ['prefix','client_id','supplier_id','is_active','notes'];
      for (const k of f) {
      ...
      await pool.query(`UPDATE number_blacklists SET ${sets.join(',')} WHERE id=$${i}`, vals);
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.delete('/api/blacklists/:id', auth, roles('super_admin','admin'), async (req, res) => {
    try {
      await pool.query('DELETE FROM number_blacklists WHERE id=$1', [req.params.id]);
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  // ============================================================
  // KEYWORD FILTERS — CRUD + list
  // ============================================================
  app.post('/api/keyword-filters/list', auth, async (req, res) => {
    try {
      let q = 'SELECT * FROM keyword_filters WHERE 1=1';
      const p = []; let i = 1;
      if (req.body?.client_id) { q += ` AND client_id=$${i++}`; p.push(req.body.client_id); }
      if (req.body?.supplier_id) { q += ` AND supplier_id=$${i++}`; p.push(req.body.supplier_id); }
      q += ' ORDER BY created_at DESC';
  ...
  app.post('/api/keyword-filters', auth, roles('super_admin','admin','support'), async (req, res) => {
    try {
      const { keyword, match_mode, client_id, supplier_id, notes } = req.body || {};
      if (!keyword) return res.status(400).json({ error: 'keyword required' });
      const r = await pool.query(
        `INSERT INTO keyword_filters (keyword, match_mode, client_id, supplier_id, is_active, action, notes)
         VALUES ($1, $2, $3, $4, true, 'block', $5) RETURNING *`,
        [keyword, match_mode || 'substring', client_id || null, supplier_id || null, notes || null]
      );
      res.json({ success: true, data: r.rows[0] });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.put('/api/keyword-filters/:id', auth, roles('super_admin','admin','support'), async (req, res) => {
    try {
      const sets = []; const vals = []; let i = 1;
      const f = ['keyword','match_mode','client_id','supplier_id','is_active','notes'];
      for (const k of f) {
        if (req.body[k] !== undefined) { sets.push(`${k}=$${i++}`); vals.push(req.body[k]); }
      }
      if (!sets.length) return res.status(400).json({ error: 'No fields' });
      sets.push(`updated_at=NOW()`);
      vals.push(req.params.id);
      await pool.query(`UPDATE keyword_filters SET ${sets.join(',')} WHERE id=$${i}`, vals);
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.delete('/api/keyword-filters/:id', auth, roles('super_admin','admin'), async (req, res) => {
    try {
      await pool.query('DELETE FROM keyword_filters WHERE id=$1', [req.params.id]);
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
...
module.exports.setModules = function(numValid, astBridge, emailSvc) {
