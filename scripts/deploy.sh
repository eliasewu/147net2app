...
#   3. Creates the PostgreSQL user + database (if not already set up)
#   4. Loads schema + all migrations
#   5. Builds the Vite SPA
#   6. Starts the Express server on :3000
# =============================================================================
set -euo pipefail
...
if command -v node &>/dev/null; then
  ...
  if [ "$NODE_VER" -lt 20 ]; then
    err "Node.js 20+ required (found $(node -v)). Install: curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash - && sudo apt-get install -y nodejs"
  fi
else
...
for migration in \
  ...
  PGPASSWORD="${DB_PASS}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" -v ON_ERROR_STOP=1 -f "src/database/${migration}.sql" 2>/dev/null || true
done
ok "All migrations loaded"

# ─── 5. Build the Vite SPA ────────────────────────────────────────────────
