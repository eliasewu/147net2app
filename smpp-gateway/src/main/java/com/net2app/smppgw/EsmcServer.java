...
public class EsmcServer {
    ...
    private void handleNewSession(SMPPServerSession session) {
        ...
        BindRequest req;
        try {
            req = session.waitForBind(30000);
        } catch (Exception e) {
            String errType = e.getClass().getSimpleName();
            log.warn("[ESMC] waitForBind {} from {}: {}", errType, remoteIp, e.getMessage());
        ...
        // Accept with the interface version the modem requested so it receives
        // the sc_interface_version in the bind response. Without this, many
        // GSM gateways stay stuck in "linking" instead of showing "Transceiver".
        // Default to SMPP 3.4 (0x34) if the modem doesn't send a version.
        org.jsmpp.bean.InterfaceVersion ifVer = req.getInterfaceVersion();
        if (ifVer == null) ifVer = org.jsmpp.bean.InterfaceVersion.IF_34;
        try {
            req.accept(req.getSystemId(), ifVer);
        } catch (Exception e) {
        ...
        // Increase enquireLink timeout from default 2s to 60s.
        // GSM modems over slow links often can't respond within 2 seconds,
        // causing the session to be dropped immediately after a successful bind.
        session.setEnquireLinkTimer(60000);

        BindType bt = req.getBindType();
        String bindMode = bt == null ? "transceiver"
            : bt == BindType.BIND_TX ? "transmitter"
            : bt == BindType.BIND_RX ? "receiver"
            : "transceiver";
        // DB constraint only allows: transceiver, transmitter, receiver
        int negotiatedVersion = ifVer.value() & 0xff;
        ...
        public void onAcceptDeliverSm(DeliverSm deliverSm) {
            try {
                if (deliverSm.isSmscDeliveryReceipt()) {
                    ...
                    // Fallback: GSM modems often use their own internal message ID
                    // which won't match our random UUID. In SMPP delivery receipts,
                    // source_addr = original destination number, dest_addr = original
                    // sender ID. So we match on getSourceAddr() for the phone number.
                    if (ps == null) {
                        String dest = deliverSm.getSourceAddr(); // original destination in receipts
                        if (dest == null || dest.isEmpty()) dest = deliverSm.getDestAddress();
                        int supplierId = ar.entityType.equals("supplier") ? ar.entityId : 0;
                        long cutoff = System.currentTimeMillis() - 300_000; // 5 min window
                        final String matchDest = dest;
                        GatewayState.PendingSubmit found = state.pendingSm.values().stream()
                            .filter(p -> p.supplierId() == supplierId
                                      && matchDest != null && matchDest.equals(p.destination())
                                      && p.submittedAt() > cutoff)
                            .findFirst().orElse(null);
                        if (found != null) {
                            // Atomic remove: only clean up if the entry still holds our snapshot
                            boolean removed = state.pendingSm.remove(found.messageId(), found);
                            if (removed) ps = found;
                            log.info("[ESME] DLR fallback match: supplier={} dest={} -> message_id={} removed={}",
                                     supplierId, matchDest, found.messageId(), removed);
                        }
                    }
                    if (ps != null) {
                        ...
                        log.warn("[ESME] DLR id={} has no matching pending submit (checked by id + fallback by dest/supplier)", smppMsgId);
                    }
                }
