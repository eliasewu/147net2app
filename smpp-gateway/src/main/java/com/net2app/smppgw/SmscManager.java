        ...
        if (inboundSession.isPresent()) {
            ...
            try {
                es.session().deliverShortMessage(
                    ...
                    text.getBytes());
                // DLR for inbound suppliers: generate a synthetic smpp_msg_id and
                // immediately push a SUBMITTED DLR so the sms_logs status updates
                // without waiting for a real delivery receipt from the modem.
                ...
                // Push synthetic DLR — modem accepted the deliver_sm, message is submitted
                dlrRouter.handleDlr(new DlrRouter.DlrPayload(
                    messageId, smppMsgId, clientId, supplierId,
                    clientCode, supplierCode, destination, sourceAddr,
                    "SUBMITTED", "000", System.currentTimeMillis()));
                return smppMsgId;
            } catch (Exception e) {
