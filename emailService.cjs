...
async function sendRateChangeEmail(opts) {
  try {
    ...
    const isNew = !opts.old_rate || opts.old_rate <= 0 || opts.change_pct === 'New';
    const changePct = opts.change_pct
      || (isNew ? 'New destination'
        : (opts.old_rate > 0 ? `${((opts.new_rate - opts.old_rate) / opts.old_rate * 100) >= 0 ? '+' : ''}${((opts.new_rate - opts.old_rate) / opts.old_rate * 100).toFixed(1)}%` : 'N/A'));
    ...
    const variables = {
      ...
      old_rate: isNew ? '—' : `€${Number(opts.old_rate).toFixed(4)}`,
