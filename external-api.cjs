...
app.post("/api/v1/sms/send", authPasswordToken, async (req, res) => {
  try {
    ...
    // ─── Blocking checks: number blacklist + keyword filter ─────────
    // Run BEFORE routing to reject blocked destinations/content early.
    // Blocked messages are logged with status='blocked' (zero cost, no billing).
    const blockCheck = await pool.checkBlockingRules(destination, message, clientId, null);
    if (blockCheck.blocked) {
      // Log the blocked message for visibility in SMS Logs
      const msgId = 'MSG' + Date.now() + Math.random().toString(36).substring(2, 6);
      await pool.query(
        `INSERT INTO sms_logs
          (message_id, client_id, client_code, sender_id, destination, message, message_parts,
           client_rate, supplier_rate, profit, currency, status, error_message, submit_time)
         VALUES ($1, $2, $3, $4, $5, $6, 1, 0, 0, 0, $7, 'blocked', $8, NOW())`,
        [msgId, clientId, c.client_code, sender_id, destination, message,
         c.currency || 'EUR', blockCheck.reason]
      );
